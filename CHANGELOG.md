...
Adds two review depths (review depth policy v1). A **standard review** (one critique pass by every required specialist, a revision and exact-final verification) becomes the default for lower-risk recommendations and revisions, at about half the reviewer calls. A **full review** keeps all three passes and is required for medications, diagnosed conditions, pregnancy or postpartum status, clinician restrictions, pain or injury, cuts and other weight-change plans, beginners or returners with health risks, new high-intensity work or large load jumps, and whenever the athlete asks. A standard review escalates to full when its final verification fails, a material finding stays open or a trigger appears. Rowan chooses conservatively, records the reason and labels each release with its actual depth; the athlete can raise the depth but never lower it. Both depths keep the same roles, rubric, score floors, independence and binding rules.

This formalizes what a tester's Rowan improvised as an undefined one-pass review. The README, How It Works, FAQ, Share and llms.txt pages now describe both depths. Changing stage counts is an evaluated amendment under the project's rules; that independent evaluation has not been run, and nothing here has been exercised on a live host.
