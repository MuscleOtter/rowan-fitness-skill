...
3. Exercise the relevant cases in `references/verification.md`. Preserve required reviewers, full-review triggers, grading floors and exact-final approval. Every applicable final specialist score must reach 8.8; existing 9.0 and stricter requirements remain. Recursion or maintenance changes require three independent critique/revision passes under `references/maintenance-rubric.md`, with every final dimension at least 8.8. Never regrade historical reports automatically or lower criteria to pass.
4. Run `python3 tools/package.py`, then `python3 tools/package.py --check`. The script builds a deterministic ZIP, complete Project knowledge document and SHA-256 list from the skill folder. It does not assess coaching quality.
5. Review every changed file and the final archive for private information. Commit the source, docs and generated distribution together. Publish a versioned GitHub release with the ZIP, `Rowan-Project-Knowledge.md` and checksum file. Keep both distribution assets on the same version; verify each against its checksum.
