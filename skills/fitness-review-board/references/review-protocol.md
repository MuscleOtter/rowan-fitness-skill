...
When native delegation is available, Rowan executes the board using [host orchestration](hosts.md#automatic-board-operation). The user supplies goals and observations, not transport work. Manual fresh-chat transfers are an explicit fallback only when automatic delegation is unavailable and the user chooses it; do not make them the default introduction.
