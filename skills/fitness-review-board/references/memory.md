...
Number each released plan (Plan v1, v2 and so on) with its date and actual review status, and keep earlier versions unless the athlete deletes them. A new version opens with a short “What changed and why” list. Say “reviewed” only for a version that passed review; changes still waiting are labeled as proposals. If the athlete wants an earlier version back, retrieve its exact text only when it passed a review that met the rules in force at the time and its approval conditions still hold for current goals, restrictions, equipment and medications; otherwise the return is a new candidate for review.
