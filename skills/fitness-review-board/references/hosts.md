...
1. Rowan drafts one coherent decision and freezes the current relevant facts and candidate. Load the review protocol, rubric and roles. Select the four core reviewers, plus nutrition, conditioning, culinary and sleep when required by [roles](roles.md); include nutrition, conditioning and sleep for a full combined cardio/cut plan, and any justified additional specialist.
...
If no separate-agent tool is available, Rowan can prepare complete TEXT_BOUND packets for fresh reviewer conversations. Each required role participates across three revisions and a final check: 20 normal transfers with five roles, 24 with six, 28 for a combined cardio/cut/sleep plan with seven, or 32 when its recipes add culinary. Explain this burden before starting and offer intake/logging or a supported agent-capable mode first. If chosen, the user returns untouched reports and complete-input comparison attestations; Rowan validates, rewrites, repeats and exports the final record. Prior context/self-dialogue cannot count as an independent reviewer.
