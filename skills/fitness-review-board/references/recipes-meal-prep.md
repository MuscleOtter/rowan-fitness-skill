...
Add Jules (`culinary`) to every review stage and exact-final verification when actionable recipes, cooking steps or meal-prep plans are included. Sage is also required, including for a recipe-only nutrition request. Core roles still apply. Jules checks feasibility, taste/texture logic, quantities/yield, equipment, timing, shopping/leftover reuse, allergens and storage/reheating. Sage independently checks nutrition sources, arithmetic, portion fit and how the meal works with the training/cut strategy. Their named findings go back to Rowan; a material issue cannot be outvoted. General explanations and logging stay on the existing lighter routes.
