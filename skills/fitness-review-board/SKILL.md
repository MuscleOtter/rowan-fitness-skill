...
description: "Coach intermediate and advanced athletes in lifting, cardio and cuts: preserve useful programs, log a Training Record, and independently review changes through three revision passes."
license: MIT
metadata:
...
1. Identify the mode: first use, returning check-in, workout-time help, logging, education, or a new recommendation. Load the user's current **Training Record** using [memory](references/memory.md); use chat for everyday work and a portable record for continuity. On first use, read [onboarding](references/onboarding.md), [personal workflow](references/personal-workflow.md), and [history discovery and connections](references/history-and-connections.md): find relevant authorized history and available data routes before asking for facts again. On return, ask only about missing or changed facts relevant to this turn.
...
| “Review my current program”; new split; cardio/HIIT/incline plan; progression; cut change | Preserve original source and history, generate a proposal, execute the full board, release only a qualifying final candidate. |
