...
Rowan is a free, MIT-licensed set of instructions and templates you add to an AI assistant. It does not include an AI subscription. Your assistant's fees, tool availability and usage limits apply; a complete review takes more work than an ordinary chat reply. [Choose your setup](SETUP.md).
...
**Does Rowan remember everything automatically?**
...
Yes, if you want it. After a plan is approved, Theo, Rowan's habits and follow-through coach, helps you start: when and where each session fits your week, reminders in apps you already use where your assistant can reach them, a workout card for your phone and a simple log sheet. At check-ins Theo works on one habit at a time. There are no penalties or streak pressure, Theo never adds make-up workouts, and Theo never changes the approved plan. The approach draws on habit research and ideas popularized by *Atomic Habits*. If you already have a system, say so and Theo stays out of the way.
...
**What do the scores prove?**

They express AI reviewers' judgments under the skill's rubric. They do not establish professional credentials, clinical validation or guaranteed results. Separate AI contexts can share errors. The published checks and their limitations—including the incomplete security scanner—are in [Validation and limits](VALIDATION.md).

**How do I update or stop using it?**

Follow [Update or remove](SETUP.md#update-or-remove). Keep your newest Training Record and let Rowan run the [non-blocking migration](../skills/fitness-review-board/references/migration.md) before relying on an older record. An unchanged plan is not re-reviewed just because the package changed; new or changed action text is versioned and reviewed. If you enabled scheduled support, disable those jobs separately; removing the skill does not remove them or delete your chats.

**What happens to an older plan when Rowan is updated?**

Its exact approved text, review history and conditions stay in place. Missing fields from newer Rowan features remain explicitly unknown or not configured, and ordinary logging continues. If the selected active released plan has no visible plan version, Rowan adds a stable label such as `Plan v1 · Legacy · Reviewed (standard; original version unknown; ID <plan ID>)` when its review evidence is verifiable, or `Plan v1 · Legacy (review status unknown; original version unknown; ID <plan ID>)` when it is not. It does not relabel every historical plan or pretend a new release occurred. Only the unknown-review legacy, held and proposal states are non-executable; a legacy plan with known review evidence remains usable while its current conditions hold. A real plan change becomes the next version with a date, review status, review depth and a short “What changed and why” note; the prior version stays available with its supersession and dependent-material record.
...
Not with one automatic switch, because each host stores Rowan separately. Choose one exact package, use it for every host, keep your Training Record outside the skill, and paste the same version-and-migration check into each fresh conversation. Keep one short receipt per host; a failed host can be restored without stopping the others. [Update everywhere in one pass](SETUP.md#update-everywhere-in-one-pass).
