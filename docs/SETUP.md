...
For automatic independent reviews, choose an available mode with separate-agent tools. Claude Code and Cowork may provide them; Rowan must check the actual session. A complete new-plan review normally takes 16 reviewer calls, or 20 when nutrition participates. Logging and retrieving unchanged, valid approved plans use lighter routes.
...
| Host | Install | Start |
|---|---|---|
| Claude Code, local | Extract the ZIP; place its `fitness-review-board` folder in `~/.claude/skills/`. | `/fitness-review-board` |
