...
For automatic independent reviews, choose a mode with separate-agent tools. Claude Code, Cowork and Codex may provide them; Rowan must check the actual session, not the product name. A reviewer also has to start from an empty context — a fork or continuation mode that inherits the conversation is not an independent reviewer, however correct its report looks. A complete new-plan review takes four calls per required reviewer: 16 for the core board, 20 for standalone sleep coaching with Wren, 24 for a full training/cardio/sleep review, 28 for a combined cardio/cut/sleep review, and 32 when that combined plan also includes recipes/meal prep reviewed by Jules. Tasks use names such as “Nico — Conditioning — Pass 1” when the host supports custom labels. Logging and retrieving unchanged, valid approved plans use lighter routes.
